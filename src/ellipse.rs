...
use crate::{common_structs::{Coord, Angle, RGBAColor}, rgba_canvas::RGBACanvas, line_seg::LineSeg};
...
impl Ellipse {
    pub fn new(center: Coord, hor_axis: f32, ver_axis: f32, color: RGBAColor) -> Ellipse {
    ...
    pub fn draw_ellipse_raster(&self, canvas: &mut RGBACanvas, is_filled: bool, edge_width: f32) {
        ...
        let long_semi_axis: f32;
        let foc_distance: f32; // distance from center to focus
        let mut sum_dist_from_foci: f32;

        let mut x: f32;
        let mut y: f32;

        let sin_alpha: f32 = f32::sin(self.angle.get_rad());
        let cos_alpha: f32 = f32::cos(self.angle.get_rad());

        if self.hor_axis >= self.ver_axis {
            long_semi_axis = self.hor_axis;

            foc_distance = f32::sqrt(long_semi_axis * long_semi_axis - self.ver_axis * self.ver_axis);
            ...
            foc_distance = f32::sqrt(long_semi_axis * long_semi_axis - self.hor_axis * self.hor_axis);
        ...
        for j in (self.center.get_y_i()-(long_semi_axis as i32 + edge_width as i32))..(self.center.get_y_i()+(long_semi_axis as i32 + edge_width as i32 + 1)) {
            for i in (self.center.get_x_i()-(long_semi_axis as i32 + edge_width as i32))..(self.center.get_x_i()+(long_semi_axis as i32 + edge_width as i32 + 1)) {
                ...
                sum_dist_from_foci =
                    f32::sqrt((x - foc_1.x()) * (x - foc_1.x()) + (y - foc_1.y()) * (y - foc_1.y())) +
                    f32::sqrt((x - foc_2.x()) * (x - foc_2.x()) + (y - foc_2.y()) * (y - foc_2.y()));
